	...
	export default {
		...
		methods: {
			...
			fetchData() {
			  ...
			  uni.request({
			    url: 'http://10.30.3.50:8386/api/iqcReport/dataIQC', // Địa chỉ API của bạn
			    method: 'GET',
			    data: {
			      page: this.startPage,
			      limit: 10,
			  ...
			  if (selected) {
			    ...
			    uni.setStorage({
			      ...
			      success: () => {
			        ...
			        uni.navigateTo({
			          url: '/pages/iqcReport/viewPage/viewMaterialIQC',
			        });
			      },
			      fail: (error) => {
			...
			showMaterialName(measurementArea) {
				switch (measurementArea) {
					...
					case 2:
						return {
							text: 'Liệu lưu trữ trong khu vực đạt'
						};
					case 3:
						return {
							text: 'Liệu chuẩn bị phát cho hiện trường'
						};
					default:
						return {
							text: 'Không xác định'
