<template>
	<view class="flex flex-col h-screen w-full bg-blue-200">
		<view class="flex justify-between gap-2 p-2">
			<view class="back" @click="goBack()"></view>
			<span class="font-bold text-[25px]">Tạo biểu nhiệt độ ngẫu nhiên máy định hình nóng</span>
			<view class=""></view>
		</view>
		<view class="bg-white rounded-t-3xl border-2 border-black mx-auto shadow-lg">
			<view class="flex text-center justify-center font-bold text-lg p-2">
				Thông tin dữ liệu
			</view>
			<view class="flex flex-col gap-4 p-2">
				...
				<view class="flex flex-col gap-2">
					...
					<view class="flex items-center border-b-2 border-black pb-1">
						<span class="pen"></span>
						<input class="w-full ml-2 outline-none font-bold text-[20px]" type="text" v-model="formData.QC"
							autofocus placeholder="Nhập MSNV" />
					</view>
				</view>
				...
				<view class="flex flex-col gap-2">
					...
					<view class="flex items-center border-b-2 border-black pb-1">
						...
						<mySelectModal class="ml-2 outline-none font-bold text-[20px]" :data="dataDepartment"
							...
							placeholder="Nhập hoặc chọn dữ liệu..." title="Danh sách lựa chọn chuyền"
			...
			<view class="flex flex-col gap-2 p-2">
					...
					<view class="grid grid-cols-3 grid-rows-1 gap-2">
						<view class="flex items-center border-b-2 border-black pb-1">
							...
							<picker class="w-full ml-2 outline-none font-semibold" :range="arrayStation"
								...
								<view class="w-full text-[20px] outline-none">
									{{ arrayStation[indexStation] || 'Chọn mã máy' }}
								</view>
							</picker>
						</view>
						...
						<view class="flex items-center border-b-2 border-black pb-1">
							...
							<picker class="w-full ml-2 outline-none font-semibold" :range="arrayStation2"
								...
								<view class="w-full text-[20px] outline-none">
									{{ arrayStation2[indexStation2] || 'Chọn mã máy' }}
								</view>
							</picker>
						</view>
						...
						<view class="flex items-center border-b-2 border-black pb-1">
							...
							<picker class="w-full ml-2 outline-none font-semibold" :range="arrayStation3"
								...
								<view class="w-full text-[20px] outline-none">
									{{ arrayStation3[indexStation3] || 'Chọn mã máy' }}
								</view>
							</picker>
						</view>
					</view>
				...
				<view class="flex flex-col gap-2">
					<span class="font-bold text-[20px]">Nhiệt độ thực tế/Temperature Actual :</span>
					<view class="grid grid-cols-3 grid-rows-1 gap-2">
						<view class="flex items-center border-b-2 border-black pb-1">
							<span class="pen"></span>
							<input class="w-full ml-2 outline-none font-semibold" type="text"
								v-model="formData.TEMP_ACTUAL_1" autofocus placeholder="Nhập mã máy 1" />
						</view>

						<view class="flex items-center border-b-2 border-black pb-1">
							<span class="pen"></span>
							<input class="w-full ml-2 outline-none font-semibold" type="text"
								v-model="formData.TEMP_ACTUAL_2" autofocus placeholder="Nhập mã máy 2" />
						</view>

						<view class="flex items-center border-b-2 border-black pb-1">
							<span class="pen"></span>
							<input class="w-full ml-2 outline-none font-semibold" type="text"
								v-model="formData.TEMP_ACTUAL_3" autofocus placeholder="Nhập mã máy 3" />
						</view>
					</view> 
				</view>
			...
			<view class="flex-1">
				<button @click="createData()" class="w-full h-full font-semibold bg-[#3acdbb] text-white">Xác
					nhận</button>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			checkboxChange(e) {
				...
				this.formData.RESULT = selectedValue
			...
			bindPickerStationChange: function(e) {
				...
				this.formData.MACHINE_1 = this.arrayStation[e.detail.value];
				console.log('Selected value: ', this.formData.MACHINE_1);
			},
			bindPickerStationChange2: function(e) {
				this.indexStation2 = e.detail.value;
				this.formData.MACHINE_2 = this.arrayStation2[e.detail.value];
				console.log('Selected value 2: ', this.formData.MACHINE_2);
			},
			bindPickerStationChange3: function(e) {
				this.indexStation3 = e.detail.value;
				this.formData.MACHINE_3 = this.arrayStation3[e.detail.value];
				console.log('Selected value 3: ', this.formData.MACHINE_3);
			},
			...
			async createData() {
				...
				try {
					...
					uni.request({
						url: `http://10.30.3.50:8386/api/iprdReport/createReportTempMachine`,
						method: 'POST',
						data: this.formData,
						success: (response) => {
							if (response.statusCode === 200) {
								...
								uni.showToast({
									title: `Tạo thất bại: ${response.data.message || "Lỗi không xác định"}`,
									icon: "none",
									duration: 2000,
								});
								this.isLoading = false;
